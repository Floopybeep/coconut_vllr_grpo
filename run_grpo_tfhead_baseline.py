...
def create_validation_snapshot(
    ...
    """
    Create a W&B table for validation snapshot.
    ...
    Args:
        validation_examples: List of dicts with keys:
            ...
            - expected_answer: str
            - is_correct: bool
        num_examples: Number of examples to include (max 30)
    ...
    for idx, example in enumerate(validation_examples[:num_examples]):
        table.add_data(
            idx + 1,
            example["question"],
            example["generated_response"],
            example["generated_response"][len(example["question"]):],
...
def main():
    ...
    if configs.load_model_path != "None":
        ...
        _raw = torch.load(configs.load_model_path, map_location='cpu')
        saved_weights = _raw.get("model_state_dict", _raw)
    ...
    for epoch in range(configs.resume, configs.num_epochs):
        ...
        if not configs.only_eval:
            ...
            for step, batch in enumerate(train_dataloader):     # batch = batch of dicts(["question", "steps", "answer", "idx"])
                ...
                # Note: Training data logging removed for cleaner wandb tracking.
                # Focus is on validation/evaluation metrics.
                ...
                if wandb_run and rank == 0:
                    if configs.coconut and scheduled_stage != 0:
                        ...
                        log_dict = {
                            ...
                            "train/term_acc": (torch.sum(term_correct).item()) / (torch.sum(term_mask).item())      # + 1 to prevent division by 0
                        }
                    else:
                        log_dict = {
                            "train/epoch": epoch + 1,
            ...
            if (
                ...
                # }
                # if rank == 0 and previous_savepoint is not None:
                    # Delete previous best model
                    # os.remove(previous_savepoint)


                if rank == 0:
                    save_path = os.path.join(save_dir, f"checkpoint_{epoch + 1}_{current_time}.pt")
                    save_model(parallel_model, save_path)
                    # torch.save(checkpoint, os.path.join(save_dir, f"checkpoint_{epoch + 1}_{current_time}.pt"))
                    print("saving model.")
                    # previous_savepoint = os.path.join(save_dir, f"checkpoint_{epoch + 1}_{current_time}.pt")
        ...
        # Collect validation examples for snapshot (only first 30)
        validation_snapshot_examples = []
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(valid_gen_dataloader):
                ...
                # Collect examples for validation snapshot
                if rank == 0 and len(validation_snapshot_examples) < 32:
                    validation_snapshot_examples.append({
                        "question": question,
                        "generated_response": text_output,
                        "extracted_response": answer_output,
        ...
        if wandb_run and rank == 0:
            # Log validation snapshot table
            if validation_snapshot_examples:
                snapshot_table = create_validation_snapshot(
                    validation_snapshot_examples,
                    num_examples=min(32, len(validation_snapshot_examples)),
                    name=f"validation_snapshot_epoch_{epoch+1}"
                )
                wandb_run.log({
                    "validation_snapshot": snapshot_table,
                    "eval/acc": cor / total,
