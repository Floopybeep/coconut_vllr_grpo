...
The well-definedness lemma, the smoothness assumption, and the practical
discussion are unchanged in substance.
...
\item For each prompt $x \sim \D$:
  ...
  \item Compute advantages
        ...
        \emph{(No $\sigma_r$ normalization.)}
  \item \emph{Update} (mask replay). For mini-epoch $e=1,\dots,E$ and
        each $k$: regenerate $\xi^{(k)}$ from stored seed (RNG-restore
        at every latent step), recompute
        $\log\pi_\theta(y^{(k)}\mid x, \xi^{(k)})$, form
        $\rho^{(k)} = \pi_\theta/\pi_{\theta_\old}$, take a gradient
        step on $\Loss^{\mathrm{GRPO}}$ from~\eqref{eq:grpo-loss}.
