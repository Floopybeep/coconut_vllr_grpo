    ...
    for epoch in range(configs.resume, configs.num_epochs):
        ...
        if not configs.only_eval:
            ...
            for step, batch in enumerate(train_dataloader):     # batch = batch of dicts(["question", "steps", "answer", "idx"])
                ...
                # Extract all scalars from outputs before deleting
                loss_scalar = loss.item() * configs.gradient_accumulation_steps
                ...
                if configs.coconut:
                    with torch.no_grad():
                        ...
                        term_acc_scalar = torch.sum(term_correct).item() / max(torch.sum(term_mask).item(), 1)
                        train_acc_scalar = torch.sum(torch.argmax(outputs.logits, dim=-1) == batch["input_ids"]).item() / (cur_bs * len(batch["input_ids"][0]))
                    del term_mask, term_correct
                ...
                if wandb_run and rank == 0:
                    if configs.coconut and scheduled_stage != 0:
                        log_dict = {
                            ...
                            "train/acc": train_acc_scalar,
                            "train/term_acc": term_acc_scalar,
                        }
                    else:
                        log_dict = {
                            "train/epoch": epoch + 1,
                ...
                pbar.set_description(
                    ...
                    f"completed (loss: {round(loss_scalar, 4)}"
                )
            pbar.close()
            dist.barrier()
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(valid_gen_dataloader):
                ...
                # Clear KV cache left over from generate()
                if hasattr(parallel_model.module, 'kv_cache'):
                    parallel_model.module.kv_cache = None
                ...
                full_ouput = tokenizer.decode(outputs[0])
                ...
                if idx < 5 and rank == 0:
                    ...
                    print(f"Full output: '{full_ouput}'")
