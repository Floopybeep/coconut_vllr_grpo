...
class Coconut(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, position_ids=None, input_embeds=None, reset_kv_cache=False, **kwargs):
        ...
        if input_ids is not None:
            ...
            for pass_idx in range(max_n_latents):           # compute latents
                ...
                next_compute_range = (
                    ...
                    (
                        ...
                        else min([mask_list[pass_idx + 1] for mask_list in latent_lists if len(mask_list) > pass_idx + 1])
                    ),
                )
            ...
            # Mask termination loss to only meaningful positions:
            # - positions labeled 1 (continue latent) are already marked
            # - last latent position per sample (terminate signal, label=0) must be added explicitly
            term_mask = termination_labels.clone()
            for batch_idx, mask_list in enumerate(latent_lists):
                if len(mask_list) > 0:
                    term_mask[batch_idx][mask_list[-1]] = 1  # include last latent (terminate)
            ...
            term_loss_fct = CrossEntropyLoss(reduction='none')
            term_loss_all = term_loss_fct(termination_logits.view(-1, 2), termination_labels.view(-1))
            term_loss_all = term_loss_all.view(termination_labels.shape)
            term_loss = (term_loss_all * term_mask).sum() / term_mask.sum().clamp(min=1)
    ...
    def generate_n(
        ...
        **kwargs
        ...
        """Generate with a forced number of latent steps.

        Like generate(), but instead of consulting the termination head,
        the first `num_latents` tokens are forced to be latent (continuous
        thought feedback), then the rest are greedy text tokens.
        """
        self.gen_forward_cnt = 0
        ...
        # First token decision: forced latent or text
        if latents_emitted < num_latents:
            next_token = self.latent_token_id
            new_token_embed = outputs.output_embeds[0, -1].view(1, 1, -1)
            latents_emitted += 1
        else:
