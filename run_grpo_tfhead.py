...
def compute_reward(generated_ids, tokenizer, ground_truth,
                            latent_id, length_penalty=0.01, accuracy_score_ratio=0.8, format_score_ratio=0.2, coconut_mode=True, is_print=False):
    """
    ...
    """
    acc_ratio = accuracy_score_ratio / (accuracy_score_ratio + format_score_ratio)
    format_ratio = 1 - acc_ratio
    ...
    for i in range(generated_ids.shape[0]):     # iterate across batch
        ...
        if num_latent == 0:
            format_score = -1
        elif text.find("<<") != -1 or text.find(">>") != -1:            # reduce format score by 0.25 for each occurance of cot step
            num_occurances = text.count("<<") + text.count(">>")
            format_score -= 0.125 * num_occurances

        # Penalise latent-only sequences: model produced latent tokens but no answer text.
        # "###" must appear in the output for a valid answer.  When absent, every rollout
        # for that question tends to score 0, producing zero GRPO advantages and no
        # gradient signal.  A negative reward guarantees a non-zero advantage signal even
        # when all rollouts fail, driving the model away from this degenerate pattern.
        if num_latent > 0 and "###" not in text and not is_numeric(answer):
            correctness = 0.0
            format_score = min(format_score - 0.5, -0.3)   # ensure clearly negative
        ...
        reward = acc_ratio * correctness + format_ratio * format_score
...
def compute_log_probs(token_ids, model_outputs, prompt_len, latent_id, pad_id):
    ...
    # For latent positions: log p(continue latent) from the termination head.
    # For text positions:   log p(go text) from the termination head  +  log p(token) from the LM head.
    #
    # With the latent-only term head, termination logits are zero at non-decision text
    # positions, giving log_softmax([0,0])[0] = log(0.5) — a constant that cancels in the
    # GRPO policy/reference ratio, contributing zero gradient.  At the critical TRANSITION
    # position (first text token, predicted by the last latent position), the term head has
    # valid logits encoding the "go text" decision.  Keeping term_lp_txt here preserves the
    # GRPO gradient signal for WHEN to stop the latent chain.
    pos_log_probs = torch.where(mask_latent, pred_term[:, :, 1], pred_term[:, :, 0] + pred_token)
...
def main():
    ...
    if rank == 0:
        import shutil
        for _src in [__file__, os.path.join(os.path.dirname(__file__), "coconut_grpo_tfhead.py"), os.path.join(os.path.dirname(__file__), "args", "gsm_vllr_grpo_coconut_tfhead.yaml")]:
    ...
    # Initialize table schemas for validation and test steps
    eval_table_columns = ["step", "batch_idx", "num_tokens", "question", "generated", "predicted_answer", "ground_truth", "reward", "accuracy"]
    test_table_columns = ["batch_idx", "num_tokens", "question", "generated", "predicted_answer", "ground_truth", "reward", "accuracy"]
    ...
    # ReduceLROnPlateau: halve LR if eval accuracy does not improve for 3 consecutive
    # eval checkpoints.  Monitors the validation accuracy computed every eval_per_steps.
    ...
    for epoch in range(configs.resume, configs.num_epochs):
        ...
        if not configs.only_eval:
            dataset_train = get_grpo_dataset(
                ...
                no_special_marker=configs.cot or configs.no_cot or configs.no_thoughts or configs.no_bot_tokens,
                shuffle=True,
                max_question_len=configs.max_new_tokens,
            )
            ...
            for step, batch in enumerate(train_dataloader):     
                ...
                # Phase 1: Rollout
                # Keep model in TRAIN mode so dropout perturbs the latent hidden states,
                # creating diverse latent trajectories across rollouts.  Text tokens and
                # termination decisions are additionally diversified via temperature sampling.
                ...
                batch = {
                    key: batch[key].to(rank) for key in batch.keys() if key not in blacklist        # set tokenized text to device(the GPU at the GPU that current code runs in - multi-GPU setting)
                }
                ...
                with torch.no_grad():
                    ...
                    del embeddings  # free GPU copy immediately
                    ...
                    with open(log_path, "a") as f:
                        ...
                        gen_len = generated.shape[1] - len_question

                        for i in range(batch_len):
                            for j in range(configs.num_rollouts):
                                idx_ans = i * configs.num_rollouts + j
                                pad_count = torch.sum((generated[idx_ans, len_question:] == tokenizer.pad_token_id).long())
                                f.write(f"Number of Tokens Generated: {gen_len - pad_count}\n")
                                f.write(f"Question: {tokenizer.decode(batch['input_ids'][idx_ans]).replace("<|endoftext|>", "").replace("\n", "")}\n")
                                f.write(f"Generated: {tokenizer.decode(generated[idx_ans, len_question:]).replace("<|endoftext|>", "").replace("\n", "")}\n")
                ...
                # Normalise rewards WITHIN each question's rollout group, not globally.
                # Layout from repeat_interleave: [A1..A_nr, B1..B_nr, ..., H1..H_nr]
                # where nr = num_rollouts.  Reshaping to (batch_len, num_rollouts) aligns
                # each row with one question's rollouts.
                #
                # Global normalisation is incorrect: it gives near-zero advantages to
                # questions where all rollouts score identically (either all correct or all
                # wrong), and creates artificial cross-question bias that distorts learning.
                # Per-group normalisation ensures each question contributes equally.
                ...
                # EVAL mode: log-prob computation must be deterministic.  The stored
                # embeddings already encode the dropout noise from the generation phase;
                # applying fresh dropout here would corrupt the log-ratio by adding
                # independent noise to BOTH policy and reference forwards, inflating
                # the measured max_log_ratio beyond the true policy drift.
                with torch.no_grad():
                    ref_model.eval()
                    ref_outputs = ref_model(input_embeds=all_outputs, token_ids=token_ids.cpu())  # all_outputs already on CPU
                ...
                # into the stored latent hidden states in all_outputs; adding fresh dropout
                # here would create a different distribution than what generated the tokens,
                # corrupting the log-ratio.  We re-enable train mode before backward() so
                # that BN/dropout statistics are tracked correctly if they exist.
                ...
                for mb_start in range(0, total_rollouts, train_mb_size):
                    ...
                    # Forward in eval mode — dropout OFF for stable, consistent log-probs.
                    # Gradients are still tracked (torch.no_grad() is NOT active here).
                    mb_new_outputs = parallel_model(input_embeds=mb_embeds, token_ids=mb_tok)
                    ...
                    # gradient under normal training; it only prevents exp() overflow which
                    # is numerically fragile in bf16 and corrupts backward passes silently.
                    ...
                    del mb_embeds, mb_loss, mb_new_lp, mb_log_ratio, mb_log_ratio_clamped, mb_ratio
                ...
                if (step + 1) % configs.eval_per_steps == 0:
                    ...
                    eval_table = wandb.Table(columns=eval_table_columns) if (wandb_run and rank == 0) else None
                    ...
                    with torch.no_grad():
                        ...
                        for idx, batch in enumerate(valid_gen_dataloader):
                            ...
                            with open(log_path, "a") as f:
                                ...
                                for i in range(batch_len):
                                    ...
                                    pad_count = torch.sum((generated[i, len_question:] == tokenizer.pad_token_id).long())
                                    f.write(f"Number of Tokens Generated: {gen_len - pad_count}\n")
                            ...
                            with open(log_path, "a") as f:
                                ...
                                for i in range(batch_len):
                                    ...
                                    # f.write(f"Generated RAW: {tokenizer.decode(generated[i, len_question:])}\n")
                                    pad_count = torch.sum((generated[i, len_question:] == tokenizer.pad_token_id).long())
                                    num_tokens = gen_len - pad_count
                                    f.write(f"Number of Tokens Generated: {num_tokens}\n")
                                    question_text = tokenizer.decode(batch['input_ids'][i]).replace("<|endoftext|>", "")
                                    generated_text = tokenizer.decode(generated[i, len_question:]).replace("<|endoftext|>", "")
                                    ...
                                    if eval_table is not None:
                                        accuracy_val = 1.0 if ans[i] == answers[i] else 0.0
                                        eval_table.add_data(
                                            step,
                                            idx * batch_len + i,
                                            int(num_tokens),
                                            question_text[:100],  # truncate for readability
                                            generated_text[:100],
                                            ans[i],
                                            answers[i],
                                            f"{rewards[i]:.2f}",
                        ...
                        # Step LR scheduler on rank 0 (single-process decision)
                        if rank == 0:
                    ...
                    if wandb_run and rank == 0:
                        wandb_run.log({
                            ...
                            "eval/results_table": eval_table,
                            "eval/lr": optimizer.param_groups[0]["lr"],
                            })
                        del eval_table
        ...
        test_table = wandb.Table(columns=test_table_columns) if (wandb_run and rank == 0) else None
        ...
        if wandb_run and rank == 0:
            wandb_run.log({
                ...
                "test/avg_n_latent": eval_avg_latent,
