...
# class CustomDatasetSFT(torch.utils.data.Dataset):
#     def __init__(self, path_json, tokenizer, latent_token_id, context_length, latent_step=0, max_latent_step=6, max_samples=1000000, verbose=False):
#         super().__init__()
#         self.tokenizer = tokenizer
#         self.context_length = context_length
#         self.latent_id = latent_token_id
#         self.latent_step = min(latent_step, max_latent_step)

#         self.dataset = self.parse_json(path_json)[:max_samples]

#     def parse_json(self, path_json):
#         with open(path_json, "r") as jsonfile:
#             contents_json = json.load(jsonfile)
#         result = []
#         for idx, contents in enumerate(tqdm(contents_json, desc="Loading dataset...")):
...
#     def tokenize_concatenated_sample(self, sample, is_train=True):
#         """
#         Concatenate question, steps, answer, then tokenize.
#         Replace the first N steps with <latent>
#         """
#         list_tokenized = self.tokenizer.encode(
#             sample["question"] + "\n", add_special_tokens=True
#         )
#         list_tokenized.extend([self.latent_id for _ in range(self.latent_step)])
#         list_tokenized.extend([
#             self.tokenizer.encode(s + "\n", add_special_tokens=False)
#             for s in sample["steps"][self.latent_step:]
#         ])
#         list_tokenized.extend(self.tokenizer.encode(
#             "### " + sample["answer"], add_special_tokens=False
#         ) + [self.tokenizer.eos_token_id])

#         if len(list_tokenized) > self.context_length:
#             print(f"WARNING: Prompt length({len(list_tokenized)}) exceeds context length({self.context_length})")
...
#         return self.tokenize_concatenated_sample(self.dataset[index])


# class CustomDataLoader(torch.utils.data.DataLoader):
#     def __init__(self, dataset, pad_token_id, latent_token_id, batch_size = 1, shuffle = None, **kwargs):
